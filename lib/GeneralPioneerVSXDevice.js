...
class GeneralPioneerVSXDevice extends Homey.Device {
  ...
  async onDiscoveryAvailable(discoveryResult) {
    ...
    this.setSettings({
      ...
      this.log(`Pioneer Device - onDiscoveryAvailable() - settings: ${err}`);
    });
  ...
  async onDiscoveryLastSeenChanged(discoveryResult) {
    ...
    if (this.device) {
      ...
      if (!this.getCapabilityValue('onoff')) {
        this.log('Pioneer Device - onDiscoveryLastSeenChanged() - Device Off');
        // Do not reconnect when device is off
  ...
  configureInputSource() {
    this.log('Pioneer Device - configureInputSource()');

    // TODO: title should be a user defined name
    // TODO: id should come from driver url page
    // TODO: first time values get from url? or directly by sending ?RGBxx
    /* const values = [{
        "id": "25",
        "title": {
          "en": "BD"
  ...
  this.setCapabilityValue('input_source', values); */
    this.log(this.getCapabilityValue('input_source'));
  }
  /*
