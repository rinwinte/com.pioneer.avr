...
class GeneralPioneerVSXDevice extends Homey.Device {
  ...
  registerCapabilityListerners() {
    ...
    this.registerCapabilityListener('speaker_playing', value => {
      if (this.getCapabilityValue('input_source') !== InputChannelDataTypeEnum.HMG) {
  ...
  async onVolumeInfo(zone, volume) {
    ...
    this.deviceLog(`onVolumeInfo() ${zone} - percentage: ${percentage}%`);
  ...
  async onSourceInfo(zone, source) {
    ...
    return this.setCapabilityValue(`input_source.${zone}`, source)
      .then(() => {
        ...
        this.driver.ready(() => {
          console.log('starting a trigger');
  ...
  async onCapabilityVolumeSet(zone, value) {
    this.deviceLog(`onCapiblityVolumeSet() - value: ${value}`);
  ...
  async BrowseHMGItem(item) {
    const inputSource = this.getCapabilityValue('input_source');
  ...
  async SetTunerFrequency(frequencyString) {
    const inputSource = this.getCapabilityValue('input_source');
    this.deviceLog(`SetTunerFrequency() - inputSource: ${inputSource}FN, item: ${frequencyString}`);

    // Check whether input source is tuner
    if (inputSource !== InputChannelDataTypeEnum.Tuner) {
