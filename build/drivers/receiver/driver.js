...
class PioneerVSXDriver extends Homey.Driver {
  ...
  onInit(...props) {
    ...
    this._inputSourceChangedTrigger = this.homey.flow.getDeviceTriggerCard('input_source_changed')
      .registerRunListener(async (args, state) => {
        this.driverLog(`FlowCardTrigger - ${args.zone} input_source_changed: ${args.input_source}, state: ${state.input_source}`);

        let inputSourceMatched = (state.input_source === args.input_source);
  ...
  triggerInputSourceChangedFlow(device, zone, source) {
    ...
    console.log(`triggeerrrrr: ${zone}, ${source}`);
