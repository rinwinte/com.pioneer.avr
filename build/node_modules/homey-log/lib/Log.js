...
class Log {
  ...
  /**
   * Severity level list
   * Debug is the least and Fatal the most serious.
   *
   */
  static Severity = {
    Debug: 'debug',
   ...
   */
  async captureMessage({ message, level = Log.Severity.Error }) {
  ...
  /**
   * Create a breadcrump for Sentry. See the raven-node documentation:
   * https://docs.sentry.io/clients/node/usage/#raven-recording-breadcrumbs
   * @param {category} category - Category of the breadcrumb
   * @param {message} message - Message of the breadcrumb
   * @param {data} data - Data object of the breadcrumb
   * @param {level} level - Severity level of the breadcrumb
   * @returns {Promise<string>|undefined}
   */
  async captureBreadcrumb(category, message, data, level = Log.Severity.Info) {
    Log._log('addBreadcrumb:', category, message, data, level);
